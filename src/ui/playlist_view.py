...
class PlaylistView(QWidget):
    ...
    def _setup_ui(self):
        ...
        # ── Bottom bar ────────────────────────────────────────────────────
        bb_widget = QWidget()
        bb_widget.setObjectName("_BottomBar")
        bb_widget.setAttribute(Qt.WidgetAttribute.WA_StyledBackground, True)
        bb_widget.setStyleSheet("#_BottomBar { background: palette(window); border-top: 1px solid palette(mid); }")
    ...
    def _save_header_state(self, *_):
        ...
        self._db.set_setting("playlist_header_state_v1", state.toBase64().data().decode())

    def _restore_header_state(self):
        if self._header_state is None:
            raw = self._db.get_setting("playlist_header_state_v1", "")
            if not raw:
                return
            try:
